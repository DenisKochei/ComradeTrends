...
export  function Home() {
  ...
  useEffect(()=>{
    const fetchPosts = async()=>{
      const res = await fetch('/api/post/getposts?category=most-trending&limit=1')
      const data = await res.json();
  ...
  useEffect(()=>{
    const fetchPosts = async()=>{
      const res = await fetch('/api/post/getposts?limit=3&category=international')
      const data = await res.json();
  ...
  useEffect(()=>{
    const fetchPosts = async()=>{
      const res = await fetch('/api/post/getposts?limit=3&category=entertainment')
      const data = await res.json();
  ...
  return (
   ...
   <Helmet>
        ...
        <>
          ...
          <div className='flex justify-center flex-wrap gap-4'>
            ...
            <div className='sm:w-1/2 w-full'>
              ...
              <h1 className=' text-2xl font-semibold text-center'>Sports News</h1>
              <div className='flex justify-center flex-wrap gap-4'>
                {sports.map((post)=>(
                  <PostBar post={post} key={post._id}/>
                ))}
    ...
    <Link to={`/search?category=politics`} className=' mb-5 text-center hover:underline text-teal-500'>
      More Politics news
    </Link>
    </div>
    }
    ...
    <div className='flex flex-col gap-3 text-center'>
      <h1 className=' text-2xl font-semibold text-center'>International News</h1>
      <div  className='flex justify-center flex-wrap h-full gap-1'>
     {international.map((post)=>( 
       <PostCard key={post._id} post={post} />
      ))}
    </div>
     <Link to={`/search?category=international`} className=' mb-5 text-center hover:underline text-teal-500'>
    ...
    <div className='flex flex-col gap-3 text-center'>
      <h1 className=' text-2xl font-semibold text-center'>Entertainment</h1>
      <div  className='flex justify-center flex-wrap h-full gap-1'>
     {entertainment.map((post)=>( 
       <PostCard key={post._id} post={post} />
      ))}
    </div>
     <Link to={`/search?category=entertainment`} className=' mb-5 text-center hover:underline text-teal-500'>
