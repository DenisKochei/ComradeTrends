  ...
  return (
    <Headroom>
      ...
      <Link to="/" className='self-center whitespace-nowrap text-sm sm:text-xs  font-semibold dark:text-white'>
        ...
        <Popover  className='flex h-auto items-center justify-center mt-6'
        ...
        <form className='w-full backdrop-blur flex-none transition-colors duration-500  border border-b-slate-500 rounded-bl-lg rounded-br-lg bg-slate-300 supports-backdrop-blur:bg-white/95 h-64 rounded-lg dark:bg-slate-900/95'>
          ...
          <Button.Group className='flex mt-2 w-full justify-start gap-0 items-center'>
            <input className='rounded-3xl w-full bg-slate-300 h-10 rounded-r-none dark:bg-slate-800 md:w-96'
              placeholder='Search...'
              type='text'
              value={searchTerm}
          ...
          <button type='submit' className='border  dark:bg-slate-800 border-slate-500 h-10 w-10 flex  justify-center items-center rounded-2xl' onClick={handleSubmit}><AiOutlineSearch className='w-5 hover:cursor-pointer  h-5'/></button>
          </div>
          </div>
        </form>
      }
    ...
    >
      ...
      <div className=' flex justify-between items-center gap-2 md:order-2'>
        {path === '/search' ? (<div></div>) : <div><AiOutlineSearch onClick={()=>SetOpen(!open)} className='w-5 hover:cursor-pointer sm:mx-4 h-5 text-slate-400'/></div>}
        <Button 
          color='gray' 
